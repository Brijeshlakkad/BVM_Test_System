...
check_session();
$login_email=$_SESSION['Studentid'];
function delete_visited_test($stu_id,$testid)
{
	global $con;
	$sql="delete from visited_test where test_id='$testid' and student_id='$stu_id'";
...
function save_result_to_database($testid,$right,$attained,$total,$remained,$total_dur)
{
	...
	$sql="Select result_attempt from results where student_id='$login_email' AND test_id='$testid'";
	$result=mysqli_query($con,$sql);
	$num=mysqli_num_rows($result);
	if($num==0)
	{
		$sql2="Insert into results(student_id,test_id,result_right,result_attended,result_total,result_left_time,result_total_time,result_attempt) values('$login_email','$testid','$right','$attained','$total','$remained','$total_dur','1')";
	...
	else
	{
		...
		$attempt++;
		$sql2="Update results SET result_right='$right',result_attended='$attained',result_left_time='$remained',result_total_time='$total_dur',result_attempt='$attempt' where student_id='$login_email' and test_id='$testid'";
		$result2=mysqli_query($con,$sql2);
		if($result2)
		{
			$status=delete_visited_test($login_email,$testid);
...
if(isset($_POST['test_completed']))
{
	...
	$sql_for_dur="Select vis_left_time from visited_test where student_id='$login_email' AND test_id='$testid'";
	...
	$sql_for_test="Select test_duration from tests where test_id='$testid'";
	...
	$sql="Select test_title,test_total_num from tests where test_id='$testid'";
	$result=mysqli_query($con,$sql);
	if($result)
	{
		...
		$title=$row[0];
		$total_que=$row[1];
		$sql2="Select que_id,que_ans from questions where test_id='$testid'";
		$result2=mysqli_query($con,$sql2);
		$right=0;
		$wrong=0;
		$attained=0;
...
if(isset($_POST['test_id']))
{
	...
	$sql="Select test_title,test_course,test_subjects,test_duration,test_total_num from tests where test_id='$testid'";
	$result=mysqli_query($con,$sql);
	if($result)
	{
		$row=mysqli_fetch_array($result);
