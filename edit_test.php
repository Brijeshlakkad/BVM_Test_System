...
if(isset($_POST['test_id']))
{
	...
	$sql="Select test_title,test_course,test_subjects,test_time from tests where test_id='$testid'";
	...
	if($result)
	{
		...
		$time=$row[3];
			?>
<div class="container well my_well_2" id="show_here">
	...
	myApp.controller("BrijController",function($scope,$http){
		...
		$scope.update_test=function(){
			$http({
				...
				data: 'test_id='+$scope.test_id+'&update_test='+$scope.t_title+'&course='+$scope.t_course+'&subjects='+$scope.t_subjects,
				headers: {'Content-Type': 'application/x-www-form-urlencoded'}
			}).then(function mySuccess(response) {
				flag = response.data;
				if(flag==1)
				{
					$scope.success_modal_val="Test updated";
